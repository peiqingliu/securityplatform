 ...
 * springsecurity认证原理
 *
 *spring security在进行认证时，会将用户名和密码封装成一个Authentication对象，
 * 在进行认证后，会将Authentication的权限等信息填充完全返回。
 * Authentication会被存在SecurityContext中，供应用之后的授权等操作使用。
 *
 * 首先会进入UsernamePasswordAuthenticationFilter并且设置权限为null和是否授权为false，
 * 然后进入ProviderManager查找支持UsernamepasswordAuthenticationToken的provider并且调用provider.authenticate(authentication);
 ...
 *
 *
 * 使用JWTAuthenticationFilter去进行用户账号的鉴权,此处都已经验证过了,只是封装了权限在UsernamePasswordAuthenticationToken对象
 ...
 *  UsernamePasswordAuthenticationFilter中，springsecurity加入了SecurityContext，既然我们现在不用了，那我们要自己定义拦截器并且加入
 * securityContext以便springsecurity作权限处理
 */
@Slf4j
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(String header, HttpServletResponse response) {
        ...
        if(tokenRedis){
            ...
            try {
                ...
                ResponseUtil.out(response, ResponseUtil.resultMap(false,403,"登录已失效，请重新登录"));
            } catch (Exception e){
