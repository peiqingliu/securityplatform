package com.security.plateform.config.security.permission;
 ...
 * @description 权限资源管理器, 为权限决断器提供支持,在认证前会把所有的资源加载进去到权限资源管理器中
 * 要实现动态的权限验证，先有对应的访问权限资源。
 * Spring Security是通过SecurityMetadataSource来加载访问时所需要的具体权限，所以第一步需要实现SecurityMetadataSource。
 */
@Slf4j
@Component
